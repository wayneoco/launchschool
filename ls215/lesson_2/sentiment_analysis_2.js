...
function sentiment(text) {
  const positiveMatches = text.match(positiveRegex).map(toLowerCase);
  const negativeMatches = text.match(negativeRegex).map(toLowerCase);
  const positiveWordCount = positiveMatches.length;
  const negativeWordCount = negativeMatches.length;
  const sentiment = positiveWordCount > negativeWordCount ? 'Positive' : 'Negative';
